...
* *Major enhancement*: empowers the app with *the ability to sync with Google Calendar*
** What it does: In Google Calendar mode, when the user  `add` / `edit` /`delete`/`clear` /`addReminder` / `deleteReminder`/ `postponeReminder` the local Events, same effect will be applied to the Google Calendar in real time.
** Justification: This feature improves the product significantly because a user might want to obtain local events from Google instead of using a blank local database, and to interact with Google Calendar thereafter. This enhancement effectively turns the app to be a Command Line Controlled version of Google Calendar.
** Highlights: This enhancement affects almost all existing commands. It required an in-depth analysis of design alternatives and the links among the commands. The implementation was extremely challenging as it required a full understanding of both local implementations as well as Google implementation. For each and every command, many format conversions are required.
** Credits: Google APIs are heavily used in this project because intensive interaction with Google Calendar is required.

* *Minor enhancement 1*: added a switch to allow user use the app in offline without any error, before the initialization of Google Calendar. The app only enters Google Calendar Mode (with sync with Google Calendar features all enabled) after the user explicitly types the command.
...
include::../UserGuide.adoc[tag=EnterGoogleCalendarMode]
...
include::../DeveloperGuide.adoc[tag=EnterGoogleCalendarMode]
