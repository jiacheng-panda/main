...
class.
The format of this command is `add event n/EVENT_NAME [s/START_DATETIME] [e/END_DATETIME] [d/DESCRIPTION] [v/VENUE] [r/REMARK]
...
==== Current Implementation
Current tag is implemented as a field in an event instead of an independent object stored in local storage. +

'edit' command can be used to manipulate tags of an event. +

'deleteTag' is used to remove a particular tag from all events in the scheduler. +

1. Implement a 'removeTag(Tag)' method in 'Scheduler'. Loop through each event, and remove the 'tag' from each event.
2. Add a new API method 'deleteTag(Tag)' in 'ModelManager'. 'ModelManager' will call 'Scheduler#removeTag(Tag)'.

==== Future Implementation
Similar to a event, all tags are stored independently from events.

.Model Class for Independent Tag Storage
image::ModelClassForTag.png[width="800"]
...
Given below is the execution flow of `Delete Tag Command'.

1. When the user inputs a command, `LogicManager` will call the `parseCommand` method in `SchedulerParser` with the user input as arguments.
2. If the user input is valid and contains the keyword `deleteTag`, `DeleteTagCommandParser` will be instantiated.
3. `SchedulerParser` will then call the `parse` method of `DeleteTagCommandParser`.
4. `DeleteCommandParser` parses the user input with the respective parsing methods.
5. `DeleteTagCommand` will be instantiated with index of the tag to be deleted.
7. `LogicManager` will then proceed to call the `execute()` method of `DeleteTagCommand`.
8. `Model` Component will then find the targeted tag(s) in internal list.
9. `Model` Component will delete the targeted tag(s).

// end::Tag Operations[]


// tag::Remark Operations[]
=== Remark Operations
It is very useful if users can add additional comments/notes about a particular event, by providing a flexible 'remark' field for each event. +
...
==== Add or edit remark for a event specified in the 'INDEX'.
Format: `remark INDEX r/[REMARK]`

// end::Remark Operations[]
